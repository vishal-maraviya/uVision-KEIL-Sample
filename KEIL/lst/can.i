#line 1 "..\\..\\DI\\M480_BSP_CMSIS_V3.05.001\\Library\\StdDriver\\src\\can.c"
...
#line 10 "..\\..\\DI\\M480_BSP_CMSIS_V3.05.001\\Library\\StdDriver\\src\\can.c"
...
static uint8_t gu8LockCanIf[3ul][2ul] = {0ul};     
...
#line 43 "..\\..\\DI\\M480_BSP_CMSIS_V3.05.001\\Library\\StdDriver\\src\\can.c"
...
static uint32_t LockIF(CAN_T *tCAN)
{
    ...
    if(tCAN == ((CAN_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x60000UL)))
        u32CanNo = 0ul;
    else if(tCAN == ((CAN_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x61000UL)))
        u32CanNo = 1ul;

    else if(tCAN == ((CAN_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x62000UL)))
        u32CanNo = 2ul;
    ...
    if((tCAN->IF[0ul].CREQ & (0x1ul << (15))) == 0ul)
    {
    ...
    if(u32FreeIfNo == 2ul)
    {
        if((tCAN->IF[1ul].CREQ & (0x1ul << (15))) == 0ul)
        {
...
static uint32_t LockIF_TL(CAN_T *tCAN)
{
    ...
    for(u32Count = 0ul; u32Count < (0x10000000ul); u32Count++)
    {
...
uint32_t CAN_GetCANBitRate(CAN_T *tCAN)
{
    ...
    u32Tseg1 = (tCAN->BTIME & (0xful << (8))) >> (8);
    u32Tseg2 = (tCAN->BTIME & (0x7ul << (12))) >> (12);
    u32Bpr   = (tCAN->BTIME & (0x3ful << (0))) | (tCAN->BRPE << 6ul);
...
int32_t CAN_BasicSendMsg(CAN_T *tCAN, STR_CANMSG_T* pCanMsg)
{
    ...
    else
    {
        ...
        tCAN->IF[0].ARB2 = ((pCanMsg->Id) & 0x1FFF0000ul) >> 16ul  | (0x1ul << (14));
    ...
    else
    {
        ...
        {
            if((tCAN->IF[0].CREQ & (0x1ul << (15))) == 0ul)
            {
                break;
            }
            else
...
int32_t CAN_BasicReceiveMsg(CAN_T *tCAN, STR_CANMSG_T* pCanMsg)
{
    ...
    if((tCAN->IF[1].MCON & (0x1ul << (15))) == 0ul)
    {
    ...
    else
    {
        ...
        if((tCAN->IF[1].ARB2 & (0x1ul << (14))) == 0ul)
        {
        ...
        pCanMsg->FrameType = (((tCAN->IF[1].ARB2 & (0x1ul << (13))) >> (13))) ? 0ul : 1ul;

        pCanMsg->DLC     = (uint8_t)(tCAN->IF[1].MCON & (0xful << (0)));
        pCanMsg->Data[0] = (uint8_t)(tCAN->IF[1].DAT_A1 & (0xfful << (0)));
        pCanMsg->Data[1] = (uint8_t)((tCAN->IF[1].DAT_A1 & (0xfful << (8))) >> (8));
        pCanMsg->Data[2] = (uint8_t)(tCAN->IF[1].DAT_A2 & (0xfful << (0)));
        pCanMsg->Data[3] = (uint8_t)((tCAN->IF[1].DAT_A2 & (0xfful << (8))) >> (8));
        pCanMsg->Data[4] = (uint8_t)(tCAN->IF[1].DAT_B1 & (0xfful << (0)));
        pCanMsg->Data[5] = (uint8_t)((tCAN->IF[1].DAT_B1 & (0xfful << (8))) >> (8));
        pCanMsg->Data[6] = (uint8_t)(tCAN->IF[1].DAT_B2 & (0xfful << (0)));
        pCanMsg->Data[7] = (uint8_t)((tCAN->IF[1].DAT_B2 & (0xfful << (8))) >> (8));
    }
...
int32_t CAN_SetRxMsgObjAndMsk(CAN_T *tCAN, uint8_t u8MsgObj, uint8_t u8idType, uint32_t u32id, uint32_t u32idmask, uint8_t u8singleOrFifoLast)
{
    ...
    else
    {
        ...
        tCAN->IF[u32MsgIfNum].CMASK = (0x1ul << (7)) | (0x1ul << (6)) | (0x1ul << (5)) |
                                      (0x1ul << (4)) | (0x1ul << (1)) | (0x1ul << (0));

        if(u8idType == 0ul)     
        {
            tCAN->IF[u32MsgIfNum].ARB1 = 0ul;
            tCAN->IF[u32MsgIfNum].ARB2 = (0x1ul << (15)) | (u32id & 0x7FFul) << 2;
        }
        else
        {
            tCAN->IF[u32MsgIfNum].ARB1 = u32id & 0xFFFFul;
            tCAN->IF[u32MsgIfNum].ARB2 = (0x1ul << (15)) | (0x1ul << (14)) | (u32id & 0x1FFF0000ul) >> 16;
        }
        ...
        tCAN->IF[u32MsgIfNum].MCON = (0x1ul << (12)) | (0x1ul << (10));
        if(u8singleOrFifoLast)
        {
            tCAN->IF[u32MsgIfNum].MCON |= (0x1ul << (7));
        }
        else
        {
            tCAN->IF[u32MsgIfNum].MCON &= (~(0x1ul << (7)));
...
int32_t CAN_ReadMsgObj(CAN_T *tCAN, uint8_t u8MsgObj, uint8_t u8Release, STR_CANMSG_T* pCanMsg)
{
    ...
    else
    {
        ...
        else
        {
            ...
            tCAN->IF[u32MsgIfNum].CMASK = (0x1ul << (6))
                                          | (0x1ul << (5))
                                          | (0x1ul << (4))
                                          | (0x1ul << (3))
                                          | (u8Release ? (0x1ul << (2)) : 0ul)
                                          | (0x1ul << (1))
                                          | (0x1ul << (0));

            tCAN->IF[u32MsgIfNum].CREQ = 1ul + u8MsgObj;

            while(tCAN->IF[u32MsgIfNum].CREQ & (0x1ul << (15)))
            {
                 
            }

            if((tCAN->IF[u32MsgIfNum].ARB2 & (0x1ul << (14))) == 0ul)
            {
                 
                pCanMsg->IdType = 0ul;
                pCanMsg->Id     = (tCAN->IF[u32MsgIfNum].ARB2 & (0x1ffful << (0))) >> 2ul;
            }
            else
            {
            ...
            pCanMsg->DLC     = (uint8_t)(tCAN->IF[u32MsgIfNum].MCON & (0xful << (0)));
            pCanMsg->Data[0] = (uint8_t)(tCAN->IF[u32MsgIfNum].DAT_A1 & (0xfful << (0)));
            pCanMsg->Data[1] = (uint8_t)((tCAN->IF[u32MsgIfNum].DAT_A1 & (0xfful << (8))) >> (8));
            pCanMsg->Data[2] = (uint8_t)(tCAN->IF[u32MsgIfNum].DAT_A2 & (0xfful << (0)));
            pCanMsg->Data[3] = (uint8_t)((tCAN->IF[u32MsgIfNum].DAT_A2 & (0xfful << (8))) >> (8));
            pCanMsg->Data[4] = (uint8_t)(tCAN->IF[u32MsgIfNum].DAT_B1 & (0xfful << (0)));
            pCanMsg->Data[5] = (uint8_t)((tCAN->IF[u32MsgIfNum].DAT_B1 & (0xfful << (8))) >> (8));
            pCanMsg->Data[6] = (uint8_t)(tCAN->IF[u32MsgIfNum].DAT_B2 & (0xfful << (0)));
            pCanMsg->Data[7] = (uint8_t)((tCAN->IF[u32MsgIfNum].DAT_B2 & (0xfful << (8))) >> (8));
...
uint32_t CAN_SetBaudRate(CAN_T *tCAN, uint32_t u32BaudRate)
{
    ...
    if((tCAN == ((CAN_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x60000UL))) || (tCAN == ((CAN_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x62000UL))))
    {
        u64PCLK_DIV = (uint64_t)(((CLK_T *) (((uint32_t)0x40000000) + 0x00200UL))->PCLKDIV & (0x7ul << (0)));
        u64PCLK_DIV = (uint64_t)(1 << u64PCLK_DIV);
    }
    else if(tCAN == ((CAN_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x61000UL)))
    {
        u64PCLK_DIV = (uint64_t)((((CLK_T *) (((uint32_t)0x40000000) + 0x00200UL))->PCLKDIV & (0x7ul << (4))) >> (4));
        u64PCLK_DIV = (uint64_t)(1 << u64PCLK_DIV);
    }
...
uint32_t CAN_Open(CAN_T *tCAN, uint32_t u32BaudRate, uint32_t u32Mode)
{
    ...
    if(u32Mode == 1ul)
    {
        CAN_EnterTestMode(tCAN, (uint8_t)(0x1ul << (2)));
    }
    else
    {
    }
...
int32_t CAN_SetTxMsg(CAN_T *tCAN, uint32_t u32MsgNum, STR_CANMSG_T* pCanMsg)
{
    ...
    else
    {
        ...
        if(pCanMsg->IdType == 0ul)
        {
            ...
            tCAN->IF[u32MsgIfNum].ARB1 = 0ul;
            tCAN->IF[u32MsgIfNum].ARB2 = (((pCanMsg->Id) & 0x7FFul) << 2) | (0x1ul << (13)) | (0x1ul << (15));
        }
        else
        {
        ...
        if(pCanMsg->FrameType)
        {
            tCAN->IF[u32MsgIfNum].ARB2 |=   (0x1ul << (13));
        }
        else
        {
            tCAN->IF[u32MsgIfNum].ARB2 &= (~(0x1ul << (13)));
        ...
        tCAN->IF[u32MsgIfNum].MCON   =  (0x1ul << (15)) | pCanMsg->DLC | (0x1ul << (11)) | (0x1ul << (7));
...
int32_t CAN_TriggerTxMsg(CAN_T  *tCAN, uint32_t u32MsgNum)
{
    ...
    else
    {
        ...
        tCAN->IF[u32MsgIfNum].CMASK = (0x1ul << (3))
                                      | (0x1ul << (2));
        ...
        tCAN->IF[u32MsgIfNum].CMASK  = (0x1ul << (7)) | (0x1ul << (2));
        tCAN->IF[u32MsgIfNum].CREQ  = 1ul + u32MsgNum;
...
int32_t CAN_SetRxMsg(CAN_T *tCAN, uint32_t u32MsgNum, uint32_t u32IDType, uint32_t u32ID)
{
    int32_t rev = (int32_t)(1UL);
    uint32_t u32TimeOutCount = 0ul;

    while(CAN_SetRxMsgObj(tCAN, (uint8_t)u32MsgNum, (uint8_t)u32IDType, u32ID, (uint8_t)(1UL)) == (int32_t)(0UL))
    {
        if(++u32TimeOutCount >= (0x10000000ul))
        {
            rev = (int32_t)((0UL));  
            break;
        }
...
int32_t CAN_SetRxMsgAndMsk(CAN_T *tCAN, uint32_t u32MsgNum, uint32_t u32IDType, uint32_t u32ID, uint32_t u32IDMask)
{
    ...
    while(CAN_SetRxMsgObjAndMsk(tCAN, (uint8_t)u32MsgNum, (uint8_t)u32IDType, u32ID, u32IDMask, (uint8_t)(1UL)) == (int32_t)(0UL))
    {
        if(++u32TimeOutCount >= (0x10000000ul))
        {
            rev = (int32_t)(0UL);
...
int32_t CAN_SetMultiRxMsg(CAN_T *tCAN, uint32_t u32MsgNum, uint32_t u32MsgCount, uint32_t u32IDType, uint32_t u32ID)
{
    ...
    for(i = 1ul; i < u32MsgCount; i++)
    {
        ...
        while(CAN_SetRxMsgObj(tCAN, (uint8_t)u32MsgNum, (uint8_t)u32IDType, u32ID, (uint8_t)u32EOB_Flag) == (int32_t)(0UL))
        {
            if(++u32TimeOutCount >= (0x10000000ul))
            {
                rev = (int32_t)(0UL);
...
int32_t CAN_Transmit(CAN_T *tCAN, uint32_t u32MsgNum, STR_CANMSG_T* pCanMsg)
{
    ...
    u32Tmp = (tCAN->TEST & (0x1ul << (2)));

    if((tCAN->CON & (0x1ul << (7))) && u32Tmp)
    {
        rev = CAN_BasicSendMsg(tCAN, pCanMsg);
    }
    else
    {
        if(CAN_SetTxMsg(tCAN, u32MsgNum, pCanMsg) == (0UL))
        {
            rev = (int32_t)(0UL);
        }
        else
...
int32_t CAN_Receive(CAN_T *tCAN, uint32_t u32MsgNum, STR_CANMSG_T* pCanMsg)
{
    ...
    {
        rev = CAN_ReadMsgObj(tCAN, (uint8_t)u32MsgNum, (uint8_t)(1UL), pCanMsg);
    }
